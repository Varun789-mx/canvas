export class CanvasDrawer {
    ...
    CreateRectangle(startX: number, startY: number, endX: number, endY: number, rounded: boolean) {
        ...
        this.ctx.strokeStyle = 'white'
        if (!rounded) {
            this.ctx.strokeRect(startX, startY, height, width)
        }
        this.ctx.roundRect(startX, startY, height, width, 5 * Math.PI);
        this.ctx.stroke();
    }
